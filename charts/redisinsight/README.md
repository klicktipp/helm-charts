...
This chart is updated for Redis Insight `3.2.0`. The main behavioral change is that preconfigured databases are now imported from a mounted JSON file via `RI_PRE_SETUP_DATABASES_PATH` instead of being posted to the legacy `/api/instance/` endpoint.

For cluster connections, the chart converts `seedNodes` into the `nodes` structure expected by Redis Insight and also promotes the first seed node to top-level `host` and `port` when they are omitted.
...
| redisinsight | object | `{"data_dir":"/data","host":"0.0.0.0","port":5540,"pre_setup_path":"/data/import/databases.json","redis_servers":[]}` | Redis Insight application settings. |
| redisinsight.host | string | `"0.0.0.0"` | Hostname value. |
| redisinsight.port | int | `5540` | Internal Redis Insight application port. |
| redisinsight.data_dir | string | `"/data"` | Mount path for Redis Insight app data. |
...
| redisinsight.redis_servers | list | `[]` | Databases to preload into Redis Insight. |
