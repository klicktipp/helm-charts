...
| transparentDNS | object | `{"bindPort":53,"captureOutput":true,"clusterDNS":{"namespace":"kube-system","selector":{"k8s-app":"kube-dns"},"serviceIP":"","upstreamService":{"annotations":{},"clusterIP":"","create":true,"name":""}},"clusterDomain":"cluster.local","customClusterDNSIP":"","enabled":false,"interceptor":{"enableRuntimeInstall":true,"image":{"pullPolicy":"IfNotPresent","repository":"alpine","tag":"3.23"},"installPackagesCommand":"apk add --no-cache iptables iproute2"},"resources":{"limits":{"memory":"128Mi"},"requests":{"cpu":"25m","memory":"128Mi"}},"securityContext":{"capabilities":{"add":["NET_ADMIN"]},"runAsGroup":0,"runAsUser":0},"setupIptables":true,"tolerations":[{"key":"CriticalAddonsOnly","operator":"Exists"},{"effect":"NoExecute","operator":"Exists"},{"effect":"NoSchedule","operator":"Exists"}]}` | Optional transparent DNS takeover mode. Experimental. Binds the existing cluster DNS Service IP locally on each node and serves it from the Recursor without changing pod DNS settings. |
| transparentDNS.enabled | bool | `false` | Enable transparent takeover of pod DNS traffic via the kube-dns/CoreDNS Service IP. Experimental. Requires workload.type=DaemonSet. This mode defaults workload.daemonSet.updateStrategy.rollingUpdate.maxSurge to 0; any explicit non-zero value will fail validation. The chart binds transparentDNS.clusterDNS.serviceIP on each node, runs PowerDNS on transparentDNS.bindPort, and programs NOTRACK/ACCEPT rules instead of NAT redirects. |
| transparentDNS.bindPort | int | `53` | Local listen/takeover port used in transparent DNS mode. Keep this at 53 for real transparent takeover. Any other value is only suitable for local debugging because pod DNS traffic to port 53 will not be transparently intercepted. |
...
| pdns.config.incoming.port | int | `5353` | Transparent DNS mode overrides the incoming port to transparentDNS.bindPort and the listen addresses to `0.0.0.0` via the `pdns.config` helper. |
